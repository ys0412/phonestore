...
<script>
  ...
  export default {
    ...
    created(){
      const _this = this
      axios.get('http://localhost:8282/phone/index').then(function (resp) {
    ...
    methods: {
      onClick(index) {
        const _this = this
        axios.get('http://localhost:8282/phone/findByCategoryType/'+this.categories[index].type).then(function (resp) {
      ...
      buy(index){
        this.show = true
        const _this = this
        axios.get('http://localhost:8282/phone/findSpecsByPhoneId/'+this.phones[index].id).then(function (resp) {
